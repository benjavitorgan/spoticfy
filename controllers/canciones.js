const conn = require("../db");

const getCanciones = (_, res) => {
    // Completar con la consulta que devuelve todas las canciones
    // Recordar que los parámetros de una consulta GET se encuentran en req.params
    // Deberían devolver los datos de la siguiente forma:
    /*
        [
            {
                "id": "Id de la canción",
                "nombre": "Nombre de la canción",
                "nombre_artista": "Id del artista",
                "nombre_album": "Id del album",
                "duracion": "Duración de la canción",
                "reproducciones": "Reproducciones de la canción"
            },
            {
                "id": "Id de la canción",
                "nombre": "Nombre de la canción",
                "nombre_artista": "Id del artista",
                "nombre_album": "Id del album",
                "duracion": "Duración de la canción",
                "reproducciones": "Reproducciones de la canción"
            },
            ...
        ]
    */
        conn.query("SELECT canciones.*, artistas.id AS id_artista FROM canciones JOIN albumes ON canciones.album = albumes.id JOIN artistas ON artistas.id = albumes.artista ", (err, results) => {
            if (err) return console.error ("Error: " + err.message);
            console.log(results);
            res.json(results);
        });
};

const getCancion = (req, res) => {
    // Completar con la consulta que devuelve una canción
    // Recordar que los parámetros de una consulta GET se encuentran en req.params
    // Deberían devolver los datos de la siguiente forma:
    /*
        {
            "id": "Id de la canción",
            "nombre": "Nombre de la canción",
            "nombre_artista": "Id del artista",
            "nombre_album": "Id del album",
            "duracion": "Duración de la canción",
            "reproducciones": "Reproducciones de la canción"
        }
    */
        conn.query("SELECT * FROM canciones WHERE id = ?", [req.params.id], (err, results) => {
            if (err) return console.error ("Error: " + err.message);
            console.log(results);
            res.json(results);
        });
};

const createCancion = (req, res) => {
    // Completar con la consulta que crea una canción
    // Recordar que los parámetros de una consulta POST se encuentran en req.body
    // Deberían recibir los datos de la siguiente forma:
    /*
        {
            "nombre": "Nombre de la canción",
            "album": "Id del album",
            "duracion": "Duración de la canción",
        }
    */
    // (Reproducciones se inicializa en 0)
    conn.query("INSERT INTO canciones (nombre, album, duracion, reproducciones) VALUES (?, ?, ?, 0)", [req.body.nombre, req.body.album, req.body.duracion], (err, results) => {
        if (err) return console.error ("Error: " + err.message);
        console.log(results);  
        res.send('Creada.');
});
};

const updateCancion = (req, res) => {
    // Completar con la consulta que actualiza una canción
    // Recordar que los parámetros de una consulta PUT se encuentran en req.body
    // Deberían recibir los datos de la siguiente forma:
    /*
        {
            "nombre": "Nombre de la canción",
            "album": "Id del album",
            "duracion": "Duración de la canción",
        }
    */
    // (Reproducciones no se puede modificar con esta consulta)
    conn.query("UPDATE canciones SET nombre = ?, album = ?, duracion = ? WHERE id = ?", [req.body.nombre, req.body.album, req.body.duracion, req.params.id], (err, results) => {
        if (err) return console.error ("Error: " + err.message);
        console.log(results);
        res.send('Actualizada.');
    });
};

const deleteCancion = (req, res) => {
    // Completar con la consulta que elimina una canción
    // Recordar que los parámetros de una consulta DELETE se encuentran en req.params
    conn.query("DELETE FROM canciones WHERE id = ?", [req.params.id], (err, results) => {
        if (err) return console.error ("Error: " + err.message);
        console.log(results);
        res.send('Eliminada.');
    });
};

const reproducirCancion = (req, res) => {
    // Completar con la consulta que aumenta las reproducciones de una canción
    // En este caso es una consulta PUT, pero no recibe ningún parámetro en el body, solo en los params
    conn.query("UPDATE canciones SET reproducciones = reproducciones + 1 WHERE id = ?", [req.params.id], (err, results) => {
        if (err) return console.error ("Error: " + err.message);
        console.log(results);
        res.send('Reproducida.');
    });
};

module.exports = {
    getCanciones,
    getCancion,
    createCancion,
    updateCancion,
    deleteCancion,
    reproducirCancion,
};
